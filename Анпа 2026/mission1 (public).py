...
if robot == 68:
    ...
    colors = {
        'red': ((104, 47, 70), (180, 255, 255)),
        'orange': ((8, 0, 0), (13, 255, 255)),
        'yellow': ((33, 0, 0), (40, 255, 255)),
        'black': ((0, 0, 1), (180, 255, 86)),
        'purple': ((119, 14, 0), (180, 255, 255)),
    }
...
def moving_average(buffer, new_value, size=10):
...
# --- ИЗМЕНЕНА ФУНКЦИЯ move_to_pinger С ПАМЯТЬЮ МАКСИМУМА ---
def move_to_pinger(p_lin=30, p_ang=8, speed=30):
    global depth, robot
    ...
    while True:
        ...
        # --- СГЛАЖИВАНИЕ ---
        d1_smooth = moving_average(d1_buffer, d1, buffer_size)
        d2_smooth = moving_average(d2_buffer, d2, buffer_size)

        # --- ЗАПОМИНАЕМ МАКСИМУМ ---
        if d1_smooth > 0:
            max_d1 = max(max_d1, d1_smooth)
        else:
            max_d1 = 0  # сигнал обнулён, начинаем заново

        if d2_smooth > 0:
            max_d2 = max(max_d2, d2_smooth)
        else:
            max_d2 = 0

        print(f"RAW: {d1:.2f} {d2:.2f} | SMOOTH: {d1_smooth:.2f} {d2_smooth:.2f} | MAX: {max_d1:.2f} {max_d2:.2f}")

        # --- УПРАВЛЕНИЕ ---
        ang_z = (max_d1 - max_d2) * p_ang  # разница для поворота
        lin_x = (max_d1 + max_d2) / 2 * p_lin  # скорость зависит от силы сигнала

        # --- ОГРАНИЧЕНИЯ ---
        ang_z = limiter(ang_z, -50, 50)
        lin_x = limiter(lin_x, 0, 60)
        ...
        # print(f"MOVE: lin={lin_x:.2f} ang={ang_z:.2f}")

        time.sleep(0.02)
        if d1 < 0.5 and d1 != 0.0:
            break
...
if __name__ == '__main__':
    ...
    tr, tl, fr, d1, d2, d3 = auv.get_hydrophone_signal()
    print(d1)
    move_to_pinger(p_ang=100, speed=20)
#
#    go_to_figure(color='purple', error_position=10, p_ang=0.5)
#    depth = 3
#    waiting_process(lambda:moving(linear_z=-30), t=8)
    while True:
        moving(linear_z=100)
