...
def moving(linear_x=0, linear_y=0, linear_z=0, angular_x=0, angular_y=0, angular_z=0):
    ...
    auv.set_motor_power(depth_motor_2, limiter(angular_z) - angular_y)
...
def second():
    global main_depth
    print('Погужась на %s метров' % main_depth)
    count = 0
    while count < 50:
        print(count)
        current_depth = auv.get_depth()
        lin_z = auv_moving.keep_depth(main_depth)
        auv_moving.moving(linear_z=lin_z)
        if abs(current_depth - main_depth) < 0.03:
            count += 1
        else:
            count = 0
...
def third():
    ...
    while count < 200:
        ...
        if biggest_orange_area > 100:
            ...
            go_to_goal1(x_goal=x, y_goal=y, goal_depth=goal_depth)
            if abs(y - 120) < 5:
                count += 1
            else:
                count = 0
